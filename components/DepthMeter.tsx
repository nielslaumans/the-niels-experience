...
type Props = { activeId: string | null; onJump: (id: string) => void };

export default function DepthMeter({ activeId, onJump }: Props) {
  const activeIndex = CHAMBERS.findIndex((c) => c.id === activeId);
  const progress = activeIndex < 0 ? 0 : ((activeIndex + 1) / CHAMBERS.length) * 100;
    ...
    <>
              ...
              {CHAMBERS.map((c) => {
                ...
                return (
                  <button
                    key={c.id}
                    onClick={() => onJump(c.id)}
                    className={`group relative rounded-full px-3 py-1.5 text-[10px] uppercase tracking-[0.25em] transition-colors ${isActive ? "text-white" : "text-white/45 hover:text-white"}`}
                  >
      ...
      <div className="fixed right-6 top-1/2 z-40 hidden -translate-y-1/2 lg:block">
        <div className="relative flex flex-col items-center gap-3 rounded-full border border-white/15 bg-black/30 px-3 py-5 backdrop-blur">
          <span className="font-mono text-[9px] uppercase tracking-[0.3em] text-bio-cyan">DPT</span>
            ...
            {CHAMBERS.map((c, i) => {
              ...
              return (
                <li key={c.id}>
                  ...
                  >
                    <span
                      className={`absolute right-6 whitespace-nowrap rounded-full bg-black/95 px-3 py-1 font-mono text-[10px] uppercase tracking-[0.3em] text-white opacity-0 transition-opacity group-hover:opacity-100 ${isActive ? "opacity-100" : ""}`}
                    >
                      {c.depth} · {c.label}
                    </span>
                    <span className="relative flex h-2.5 w-2.5">
                      ...
                      <span
                        className={`relative inline-flex h-2.5 w-2.5 rounded-full transition-all ${isActive ? "scale-125 bg-bio-cyan shadow-[0_0_14px_rgba(0,240,255,0.85)]" : isPassed ? "bg-bio-aqua/70" : "bg-white/25 hover:bg-white"}`}
                      />
                    </span>
                  </button>
                </li>
          ...
          <span className="font-mono text-[9px] uppercase tracking-[0.3em] text-white/35">∞</span>
        </div>
      </div>
    </>
  );
