...
export default function Hero({ onStart }: Props) {
  return (
    <section id="aankomst" className="relative isolate min-h-screen w-full overflow-hidden">
      <div aria-hidden className="absolute inset-0 bg-gradient-to-b from-[#a4ddf0]/40 via-[#4fb8d9]/15 to-transparent" />
      <motion.div
        aria-hidden
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      <div className="relative mx-auto flex min-h-screen max-w-6xl flex-col items-center justify-center px-6 py-24">
        <div className="grid w-full items-center gap-12 lg:grid-cols-[1fr,auto,1fr]">
          ...
          >
            ...
            <h1 className="mt-6 font-display text-6xl font-bold leading-[0.9] tracking-tight text-white sm:text-7xl">
              ...
              <span className="block">
                <span className="outline-text">{hero.greetingName}</span>
              </span>
            </h1>
          </motion.div>
          ...
          >
            <div className="relative h-64 w-64 sm:h-80 sm:w-80">
              <div aria-hidden className="absolute inset-0 rounded-full bg-bio-cyan/30 blur-2xl animate-glowPulse" />
              ...
              <div className="absolute inset-4 overflow-hidden rounded-full bg-ocean-deep">
                ...
                />
                <div className="hidden h-full w-full items-center justify-center bg-gradient-to-br from-ocean-sunlit to-ocean-deep font-display text-6xl font-bold text-white/30">
                  N
                </div>
              </div>
              <motion.span
          ...
          >
            <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-bio-cyan glow-cyan">{hero.subBadge}</p>
            <p className="mt-4 max-w-md text-base leading-relaxed text-white/85 sm:text-lg">{hero.intro}</p>
          </motion.div>
        </div>
        ...
        >
          ...
          >
            ...
            <svg className="relative z-10 ml-3 h-4 w-4 transition-transform group-hover:translate-y-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <path d="M12 5v14M5 12l7 7 7-7" />
            </svg>
          </button>
          <p className="font-mono text-[10px] uppercase tracking-[0.4em] text-white/50">{hero.scrollHint}</p>
          <motion.div
            animate={{ y: [0, 10, 0] }}
          ...
          {[...Array(2)].map((_, copy) => (
            <div key={copy} className="flex shrink-0 items-center gap-12 pr-12">
              {site.marquee.map((word, i) => (
                <span key={`${copy}-${i}`} className="font-display text-xs uppercase tracking-[0.4em]">
                  ◆ {word}
                </span>
              ))}
            </div>
